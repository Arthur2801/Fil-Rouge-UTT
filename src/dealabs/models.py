"""
Modèles de données pour l'API Dealabs.

Ce module définit les classes représentant les entités Dealabs :
    - Deal: Représente une offre/deal sur Dealabs
    - Comment: Représente un commentaire sur un deal
    - Merchant: Représente un marchand

Ces classes permettent de structurer les données extraites de l'API Dealabs
et facilitent leur manipulation et sauvegarde.
...
def parse_timestamp(value):
    """
    Parse un timestamp qui peut être un Unix timestamp (int/float) ou une chaîne ISO.

    Args:
        value (int|float|str): Timestamp à parser

    Returns:
        datetime: Objet datetime ou None si le parsing échoue
    """
...
class Deal:
    """
    Représente un deal/offre sur Dealabs.

    Attributes:
        thread_id (str): Identifiant unique du deal
        title (str): Titre du deal
        url (str): URL du deal sur Dealabs
        price (float): Prix du produit
        category (str): Catégorie principale du deal
        merchant (str): Nom du marchand
        temperature (int): Score de température (votes)
        temperature_level (str): Niveau de température (hot/cold)
        is_hot (bool): Indique si le deal est "hot"
        created (datetime): Date de création du deal
    """

    def __init__(self, data):
        """
        Initialise un Deal à partir des données de l'API Dealabs.

        Args:
            data (dict): Dictionnaire contenant les données du deal
        """
        self.data = data
    ...
    def __repr__(self):
        """Représentation textuelle du Deal."""
        return f"Deal(id={self.thread_id}, title='{self.title}')"

    def to_dict(self):
        """
        Convertit le Deal en dictionnaire pour sauvegarde/export.

        Returns:
            dict: Dictionnaire contenant tous les attributs du deal
        """
        return {
